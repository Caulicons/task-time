...
export const isTaskSelected = selectorFamily({
  ...
  get:
    ...
    ({ get }): boolean | undefined => {
      const isSelected = get(tasksState).find(
        (task) => task.id === taskID
      )?.selected;
      return isSelected;
    },
...
export const selectedTask = selector({
  ...
  set: ({ get, set }, taskID: DefaultValue | taskInterface) => {
    const newTasks = get(tasksState).map((task) => {
      const { id } = taskID as taskInterface;
  ...
  set:
    ...
    ({ get, set }, updatedTask: any) => {
      const updateTasks = get(tasksState).map((task) => {
        ...
        if (task.time === "00:00")
          return { ...updatedTask, complete: true, isRunning: false };
...
export const restaureTask = selector<
  taskInterface | null | string | DefaultValue
>({
  key: "restaureTask",
  get: () => {
    return null;
...
export const taskDeleted = selector<taskInterface | null | string>({
  ...
  set: ({ get, set }, taskID) => {
    const handleID = taskID as string;
    const newArrTask = get(tasksState).filter(
      (task) => task.id !== handleID
    ) as taskInterface[];
