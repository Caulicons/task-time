...
import { finishedTasks, isTaskSelected, selectedTask } from "../../../../Atoms/tasksState";
...
const Task = ({ id, title, time, describer, ...props }: taskProps) => {
  ...
  const isSelected = useRecoilValue<boolean | undefined>(isTaskSelected(id)) as boolean
  ...
  return (
    ...
    ${isComplete ? 'bg-green-300'
        : isSelected ? 'bg-indigo-400' : 'bg-primary'
      }
      h-fit
      w-full
      flex
